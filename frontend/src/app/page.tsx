  ...
  const handleChatSubmit = async () => {
    ...
    try {
      ...
      setChatHistory(prev => [...prev, { 
        type: 'ai', 
        message: data.response || data.error || "Sorry, I couldn't process your request.", 
        timestamp: new Date() 
      }]);
    } catch (error) {
              ...
              {mealPlan.bmr_data && (
                <div style={{ marginBottom: '2rem', padding: '1.5rem', backgroundColor: '#f0f9ff', borderRadius: '8px', border: '1px solid #bfdbfe' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', fontSize: '0.875rem' }}>
                    ...
                    <div>
                      <span style={{ fontWeight: '500' }}>Protein:</span> {Math.round(mealPlan.bmr_data.recommended_calories * 0.25 / 4)}g
                    </div>
                    <div>
                      <span style={{ fontWeight: '500' }}>Carbs:</span> {Math.round(mealPlan.bmr_data.recommended_calories * 0.45 / 4)}g
                    </div>
                    <div>
                      <span style={{ fontWeight: '500' }}>Fats:</span> {Math.round(mealPlan.bmr_data.recommended_calories * 0.30 / 9)}g
                    </div>
                  </div>
                </div>
              )}

              {mealPlan.meal_plan && mealPlan.meal_plan.meals && (
                <div>
                  <h3 style={{ fontWeight: '600', marginBottom: '1.5rem', fontSize: '1.25rem' }}>Your Personalized Meal Plan</h3>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
                    {Object.entries(mealPlan.meal_plan.meals).map(([mealType, meal]: [string, any]) => {
                      return (
                        <div key={mealType} style={{ ...cardStyle, padding: '1.5rem' }}>
                          ...
                          <div>
                            <h5 style={{ fontWeight: '600', fontSize: '1rem', marginBottom: '0.5rem', color: '#374151' }}>
                              {meal.name}
                            </h5>
                            ...
                            <div style={{ 
                              ...
                              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.5rem' }}>
                                <div><span style={{ fontWeight: '500' }}>Calories:</span> {Math.round(meal.calories)}</div>
                                <div><span style={{ fontWeight: '500' }}>Protein:</span> {Math.round(meal.protein)}g</div>
                                <div><span style={{ fontWeight: '500' }}>Carbs:</span> {Math.round(meal.carbs)}g</div>
                                <div><span style={{ fontWeight: '500' }}>Fats:</span> {Math.round(meal.fats)}g</div>
                              </div>
                            </div>
                            ...
                            {meal.ingredients && meal.ingredients.length > 0 && (
                              <div style={{ marginBottom: '1rem' }}>
                                <h6 style={{ fontWeight: '500', fontSize: '0.875rem', marginBottom: '0.5rem', color: '#374151' }}>
                                  Ingredients:
                                </h6>
                  ...
                  <div style={{ ...cardStyle, padding: '1.5rem', marginTop: '2rem', backgroundColor: '#f0f9ff' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', fontSize: '0.875rem' }}>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontWeight: '600', fontSize: '1.125rem', color: '#1f2937' }}>
                          {Math.round(mealPlan.meal_plan.total_calories)}
                        </div>
                        <div style={{ color: '#6b7280' }}>Total Calories</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontWeight: '600', fontSize: '1.125rem', color: '#1f2937' }}>
                          {Math.round(mealPlan.meal_plan.total_protein)}g
                        </div>
                        <div style={{ color: '#6b7280' }}>Protein</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontWeight: '600', fontSize: '1.125rem', color: '#1f2937' }}>
                          {Math.round(mealPlan.meal_plan.total_carbs)}g
                        </div>
                        <div style={{ color: '#6b7280' }}>Carbs</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontWeight: '600', fontSize: '1.125rem', color: '#1f2937' }}>
                          {Math.round(mealPlan.meal_plan.total_fats)}g
                        </div>
          ...
          <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem' }}>
            <div style={cardStyle}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <h3 style={{ fontSize: '1.5rem', fontWeight: '600', margin: 0 }}>Chat with AI Nutritionist</h3>
                <button
                  style={{
                    background: 'none',
                    border: 'none',
                ...
                {chatHistory.length === 0 ? (
                  ...
                  chatHistory.map((chat, index) => (
                    <div key={index} style={{ 
                      ...
                      padding: '0.75rem',
                      borderRadius: '8px',
                      backgroundColor: chat.type === 'user' ? '#dbeafe' : '#ffffff',
                      border: chat.type === 'user' ? '1px solid #93c5fd' : '1px solid #e5e7eb'
                    }}>
                      <div style={{ 
                        fontWeight: '500', 
                        marginBottom: '0.25rem',
                        color: chat.type === 'user' ? '#1e40af' : '#059669'
                      }}>
                        {chat.type === 'user' ? 'You' : 'AI Nutritionist'}
                      </div>
                      <div style={{ color: '#374151' }}>{chat.message}</div>
                    </div>
                  ))
                )}
                {chatLoading && (
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <input
                  ...
                  placeholder="Ask about nutrition, recipes, meal planning..."
                  style={{
                    ...inputStyle,
                    flex: 1,
                    margin: 0
